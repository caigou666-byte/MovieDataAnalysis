setInterval(function () {
  ...
  var timeStr = "北京时间 " + hours + ":" + minutes + ":" + seconds;
...
function renderData(data1, data2) {
  ...
  cardContainer.id = "parent";
  cardContainer.classList.add("masonry");
  window.data1_ = [];
  window.data2_ = [];
  data1.forEach((item, index) => {
    ...
    if (isObject(item)) {
      ...
      data1_ = JSON.parse(item);
    }
    if (isObject(data2[index])) {
      data2_ = data2[index];
    } else {
      data2_ = JSON.parse(data2[index]);
    }
    console.log(data1_);
    console.log(data2_);
    window.data1_.push(data1_);
    window.data2_.push(data2_);
    const card = document.createElement("div");
    card.classList.add("masonry-item", "child");
    card.innerHTML = `
    <div class="card mb-3" style="width:400px;height:320px;">
        <div class=" d-flex justify-content-center">
            <div class="">
                <img referrer="no-referrer|origin|unsafe-url" src='${data2_["picSrc"]["img"]}' class="img-fluid rounded-start text-center align-items-center">
            </div>
            <div class="">
                <div class="card-body">
                    <h3 class="card-title font-weight-bold">${data1_["movieName"]}</h3>
                    <div class="mb-3">
                    <span class="badge rounded-pill text-bg-danger">票房：${data1_["boxOffice"]}</span>
                    <span class="badge rounded-pill text-bg-danger">评分：${data2_["rank"]}</span>                    
                    </div>

                    <div class="alert alert-warning  fade show" role="alert">
                        <div>
                            <strong>主演:</strong>${data1_["cast"]}
                        </div>
                        <div>
                            <strong>上映时间:</strong>${data1_["time"]}
                        </div>
                        <div>
                            <strong>影片类型:</strong>${data1_["type"]}
                        </div>
                    </div>
...
`;
  ...
  //设置瀑布流
  var container_ = document.querySelector(".masonry");
  var masonry = new Masonry(container_, {
  ...
  const children = parent.querySelectorAll('div[class*="child"]');
  children.forEach((item, index) => {
    item.addEventListener("click", function () {
      // 输出点击的子元素索引值
      // console.log(index);
      var info_alert = new bootstrap.Modal("#info-alert");
      info_alert.show();
      $("#movieName").text(window.data1_[index]["movieName"]);
      $("#PlotSummary").text(window.data2_[index]["PlotSummary"]);
      $("#info").empty()
      $("#comment").empty()
      // 使用 for...in 循环提取对象的键和值
      for (let key in window.data2_[index]["info"]) {
        // 提取对象的键和值
        let p = "<p><strong>" + key + "：</strong>" + window.data2_[index]["info"][key] + "</p>"
        $("#info").append(p)
      }
      window.data2_[index]["comment"].forEach((item, index) => {
        let comment = `<div class="alert alert-light" role="alert">
        `+ item + `
      </div>`
...
function readDataFromDatabase() {
  $("#modalTitle").text("正在读取数据库，加载数据中···");
  var myModal = new bootstrap.Modal("#myModal");
  myModal.show();
  getData("/submitTask_getMovieDataFromDatabase", "POST")
    .then(function (response) {
      let randomID = response["randomID"];
      const timer = setInterval(function () {
        getData("/checkTaskStatus/" + randomID, "GET").then(function (
          response
        ) {
          if (response["error"] === undefined) {
            ...
            console.log(data);
            renderData(data["data1"], data["data2"]);
          }
        });
      }, 1000);
    })
...
function drawChart() {
  if (
    window.data1_ === null ||
    window.data1_ === undefined ||
    window.data2_ === null ||
    window.data2_ === undefined
  ) {
    var alert = new bootstrap.Modal("#alert");
    alert.show();
  }
  $("#chart1").show();
  $("#chart2").show();
  ...
  var option1;
  let array_ = [];
  for (let i = 0; i < window.data1_.length; i++) {
    let movieName = window.data1_[i]["movieName"];
    let boxOffice = parseFloat(
      window.data1_[i]["boxOffice"].replace(/亿票房/, "")
    );
    let rank = parseFloat(window.data2_[i]["rank"]);
    array_.push({ movieName: movieName, 票房: boxOffice, 评分: rank });
  }
  // console.log(array);
  ...
  option1 = {
    ...
    xAxis: [
      {
        type: "category",
        data: array_.map((movie) => movie.movieName),
        axisPointer: {
          type: "shadow",
        },
        axisLabel: {
    ...
    series: [
      {
        ...
        data: array_.map((movie) => movie.票房),
      },
      {
        name: "评分",
        type: "line",
        ...
        data: array_.map((movie) => movie.评分),
      },
    ],
  };
  ...
  // 准备词云图数据
  let tagsString = "";
  window.data1_.forEach((item, index) => {
    console.log(item["type"]);
    tagsString += item["type"] + " ";
  });
  // 将字符串按照空格分隔成多个标签字符串
  const tagsStringArray = tagsString.split(" ");

  // 将多个标签字符串转换成一个包含多个标签的数组
  const tagsArray = tagsStringArray.reduce((result, tags) => {
    const tagsList = tags.split("、").map((tag) => tag.trim());
    return result.concat(tagsList);
  }, []);

  // 将数组中的每个标签转换成一个包含 name 和 value 属性的对象，并按照 name 属性分组统计 value 属性值
  const tagsData = tagsArray.reduce((result, tag) => {
    const targetTag = result.find((t) => t.name === tag);
    if (targetTag) {
      targetTag.value += 1;
    } else {
      result.push({ name: tag, value: 1 });
    }
    return result;
  }, []);
...
function startCrawlerReadingData() {
  $("#modalTitle").text(
    "正在启动爬虫，数据爬取中···(如果长时间未响应，请刷新浏览器！)"
  );
  var myModal = new bootstrap.Modal("#myModal");
  myModal.show();
  getData("/submitTask_startCrawlerToGetMovieDataFromMaoyan", "POST")
    .then(function (response) {
      let randomID = response["randomID"];
      const timer = setInterval(function () {
        getData("/checkTaskStatus/" + randomID, "GET").then(function (
          ...
          if (response["error"] === undefined) {
            ...
            // myModal.hide();
            window.data1 = response["result"];
            console.log(data1);
            //获取豆瓣数据
            getData(
              "/submitTask_startCrawlerToGetMovieDataFromDouban",
              "POST"
            ).then(function (response) {
              let randomID = response["randomID"];
              const timer = setInterval(function () {
                getData("/checkTaskStatus/" + randomID, "GET").then(function (
                  ...
                  if (response["error"] === undefined) {
                    clearInterval(timer);
                    myModal.hide();
                    window.data2 = response["result"];
                    console.log(data2);
                    renderData(window.data1, window.data2);
                  }
                });
              }, 1000);
            });
